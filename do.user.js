...
function init(){
    ...
    var base64pass=btoa(password);
        base64pass=base64pass.replace(/=+/,"");
    var base64obfsparm=btoa(obfs_param);
        base64obfsparm=base64obfsparm.replace(/=+/,"");

    var config_ssr=ip+":"+server_port+":"+protocol+":"+method+":"+obfs+":"+base64pass;//+"/?obfsparam="+btoa(obfs_param);
        config_ssr=btoa(config_ssr);
        config_ssr=config_ssr.replace(/=+/,"");
        config_ssr=config_ssr.replace(/\+/,"-");
        config_ssr=config_ssr.replace(/\//,"_");
    var ssr_urlsafebase64="ssr:\/\/"+config_ssr;
    ...
    var newCol_2=newTabrow.insertCell(0);//添加命令内容
        newCol_2.innerHTML="</div><div class=\"c-info__data\"><div id=\"ember723\" class=\"u-mgt13 u-mgb13 ember-view\"><ul class=\"list list-unstyled\"> <li>"+ssh+ "</li> <li>"+ssr_config+"<li></ul></div>";
    var newCol_1=newTabrow.insertCell(1);//添加列名称
        newCol_1.innerHTML="";
    var newCol_3=newTabrow.insertCell(2);
        newCol_3.innerHTML=app_name;
...
//添加按钮，用来触发调用函数
function button(){
   //jQuery("#Tabs u-mb-4 droplet-tabs").append('<li id="I_am_a_button" class="is-active ember-view"><a>点我</a></li>');
   var iamabutton=document.createElement("li");
    iamabutton.innerHTML='<li id="I_am_a_button"><a><font color="blue">点我</font></a></li>';
   document.getElementsByClassName("Tabs u-mb-4 droplet-tabs")[0].appendChild(iamabutton);
   jQuery("#I_am_a_button").click(function(){
        init();
    });
}
